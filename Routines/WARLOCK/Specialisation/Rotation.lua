...
-- 【新增】版本信息
local ROTATION_VERSION = "1.8.3" -- 更新版本号
...
local targetSwitchCooldown = 0.5 -- 目标切换冷却时间（秒）
...
-- 【重写】自定义目标选择函数 - 真正实现自动切换目标
local function GetAndSwitchCustomTarget()
    -- 检查自定义目标选择是否启用
    if not IsToggleEnabled("CustomTargetToggle") then
        return target
    end
    ...
    -- 获取当前目标
    local currentTarget = Aurora.UnitManager:Get("target")

    -- 检查当前目标是否有效
    local isCurrentTargetValid = currentTarget.exists and
        currentTarget.alive and
        currentTarget.combat and
        currentTarget.los and
        not currentTarget.aura(HAVOC_DOT_ID)
    ...
    Aurora.enemies:within(40):each(function(enemy)
        if enemy.exists and enemy.alive and enemy.combat and enemy.los then
            -- 检查是否没有浩劫dot
    ...
    -- 如果找到了更好的目标，切换目标
    if bestTarget and bestTarget.exists then
        -- 使用Aurora框架的方法切换目标
        if player.settarget(bestTarget) then
            print("自动切换目标到: " .. bestTarget.name)
...
local function moveSpell()
    local currentTarget = GetAndSwitchCustomTarget() -- 【修改】使用新的目标切换函数

    if not (spells.conflagrate:ready() and spells.conflagrate:castable(currentTarget)) then
        return false
    end
    if spells.conflagrate and spells.conflagrate:ready() and spells.conflagrate:castable(currentTarget) then
        spells.conflagrate:cast(currentTarget)
        return true
    end
    ...
    if spells.shadowburn and spells.shadowburn:ready() then
        return spells.shadowburn:cast(currentTarget)
    end
end
...
local function SmartShadowburn()
    ...
    -- 【修改】使用新的目标切换函数
    local currentTarget = GetAndSwitchCustomTarget()

    -- 条件1：目标即将在5秒内死亡 - 使用暗影灼烧获取碎片返还
    if currentTarget.exists and currentTarget.alive and currentTarget.combat then
        local ttd = TimeToDieRR(currentTarget, 0)
        if ttd <= 5 then
            if spells.shadowburn:castable(currentTarget) then
                return spells.shadowburn:cast(currentTarget)
            end
        end
    end
    ...
    if soul_shard >= 4 then
        ...
        local cannotCastChaosBolt = active_enemies > aoeThreshold or
            ...
            not spells.chaos_bolt:castable(currentTarget)
    ...
    if currentTarget.exists and currentTarget.alive and currentTarget.hp < 30 then
        if spells.Kureshuaijie:isknown() then
            local shouldUseInExecute = soul_shard >= 2 or player.moving or charges >= maxCharges - 1

            if shouldUseInExecute and spells.shadowburn:castable(currentTarget) then
                return spells.shadowburn:cast(currentTarget)
            end
        end
    end
...
local function Dps()
    ...
    if spells.cataclysm:isknown() and spells.cataclysm:ready() and spells.cataclysm:castable(player) then
        if active_enemies >= cataclysmThreshold and shouldUseAOE and ShouldUseCooldown() and ShouldUseCataclysm() then
            spells.cataclysm:smartaoe(currentTarget, {
    ...
    if not spells.wither:isknown() then
        if spells.Immolate and spells.Immolate:ready() and spells.Immolate:castable(currentTarget) then
            if not currentTarget.aura(157736) or currentTarget.auraremains(157736) < 3 then
                spells.Immolate:cast(currentTarget)
                return true
            end
        end
    end
    -- actions.assisted_combat+=/wither,if=!dot.immolate.ticking&!talent.wither --枯萎
    if spells.wither and spells.wither:ready() and spells.wither:castable(currentTarget) then
        if not currentTarget.aura(auras.immolate) and not talents.wither_talent then
            spells.wither:cast(currentTarget)
            return true
        end
    end
    ...
    if spells.wither and spells.wither:ready() and spells.wither:castable(currentTarget) then
        if not currentTarget.aura(445474) and talents.wither_talent then
            spells.wither:cast(currentTarget)
            return true
        end
    end
    ...
    if IsToggleEnabled("BigBurstToggle") and ShouldUseCooldown() then --cooldown
        ...
        if spells.summon_infernal:isknown() and spells.summon_infernal:ready() and spells.summon_infernal:castable(player) then
            spells.summon_infernal:smartaoe(currentTarget, {
                offsetMin = 0,  -- 最小偏移距离（避免贴脸）
                offsetMax = 40, -- 最大偏移距离（限制释放范围）
                filter = function(unit, distance, position)
                    -- 过滤条件：只统计存活的敌人
    ...
    if spells.infernal_bolt and spells.infernal_bolt:ready() and spells.infernal_bolt:castable(currentTarget) then
        if player.aura(433891) and player.auraremains(433891) <= 5 then
            spells.infernal_bolt:cast(currentTarget)
            return true
        end
    end

    -- actions.assisted_combat+=/incinerate,if=buff.infernal_bolt.up&buff.infernal_bolt.remains<=5 --烧尽
    if spells.incinerate and spells.incinerate:ready() and spells.incinerate:castable(currentTarget) then
        if player.aura(433891) and player.auraremains(433891) <= 5 then
            spells.incinerate:cast(currentTarget)
            return true
        end
    end
    ...
    if spells.channel_demonfire and spells.channel_demonfire:ready() and spells.channel_demonfire:castable(player) then
        if currentTarget:distanceto(player) <= 40 then
    ...
    if spells.soul_fire and spells.soul_fire:ready() and spells.soul_fire:castable(currentTarget) then
        if player.aura(457555) and soul_shard <= 4 then --屠戮buff 457555 5个碎片自动获得
            spells.soul_fire:cast(currentTarget)
            return true
        end
    end
    ...
    if IsToggleEnabled("RuinationToggle") then
        ...
        if spells.ruination and spells.ruination:ready() and spells.ruination:castable(currentTarget) then
            if soul_shard >= 4 and active_enemies <= aoeThreshold then
                spells.chaos_bolt:cast(currentTarget)
                return true
            end
        end
    ...
    if spells.chaos_bolt and spells.chaos_bolt:ready() and spells.chaos_bolt:castable(currentTarget) then
        if soul_shard >= 4 and active_enemies <= aoeThreshold then
            spells.chaos_bolt:cast(currentTarget)
            return true
        end
    ...
    if spells.dimensional_rift and spells.dimensional_rift:ready() and spells.dimensional_rift:castable(currentTarget) then
        spells.dimensional_rift:cast(currentTarget)
        return true
    end
...
-- 【新增】自定义目标选择宏
Aurora.Macro:RegisterCommand("customtarget", function()
    if Aurora.Rotation.CustomTargetToggle then
        local current = Aurora.Rotation.CustomTargetToggle:GetValue()
        Aurora.Rotation.CustomTargetToggle:SetValue(not current)
        print("自定义目标选择: " .. (not current and "启用" or "禁用"))
    end
end, "切换自定义目标选择状态")
